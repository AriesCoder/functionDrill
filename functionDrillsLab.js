...
let compareNums = (x,y) => {
  ...
  // }else{
  //   return x
  let result = (Number(x) > Number(y))? Number(x) : Number(y) 
...
let sum = console.log('The addition is ' + add("9",3))
...
console.log(exclaim("Problem 7 is expression"))
...
console.log(exclaimTwo("Problem 8 is Arrow"))
 //arrow*******
// declaration
// expression
...
console.log(exclaimThree("gtbtbgv"))
...
console.log(exclaimFour("Problem 9 is Declaration"))
...
function thatsOdd(aNumber){
  let result = (aNumber % 2 === 0)? "That's not odd!" : 'That is odd indeed!'
  console.log(result)
}
...
//CODE HERE
function theEliminator(arrOfStr, str){
  for(let i = 0; i < arrOfStr.length; i++){
    (arrOfStr[i] === str)? arrOfStr.splice(i, 1): arrOfStr
  }
  return arrOfStr
}
...
function emailCheck(email){
  ...
  console.log("change to Str: " + emailStr)
  let trimEmail = emailStr.trim()
...
//CODE HERE
function buyChoc(goldAmount){
  let chocFrog = 3
  let amountOfChoc = goldAmount / chocFrog
  return amountOfChoc

}
let totalFrogs = console.log(buyChoc(14))
...
function buyChoc2(goldAmount){
  let chocFrog = 3
  let amountOfChoc = Math.floor(goldAmount / chocFrog)
  return amountOfChoc
}
let totalFrogs2 = console.log(buyChoc2(14))
...
function isAscendingArr(arr){
  for(let i = 0; i < arr.length - 1 && isTrue; i++){
    if(arr[i] < arr[i+1]){
      isTrue 
    }else{
      isTrue =!isTrue
...
let arrayIsAscending = console.log(isAscendingArr(sampleArray))
